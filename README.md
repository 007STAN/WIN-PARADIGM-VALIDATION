...
**See:** [White_Papers/Derivation of the 8×8 Torus Substrate](White_Papers/Derivation%20of%20the%208%C3%978%20Torus%20Substrate.md)
...
The within-class mass ordering gaps are:
...
The residual in the mass ordering is:
...
| 13 | Mesh-native mass formula | Derived |
...
| Item | Status |
|---|---|
| kL from the mesh (not Weinberg angle) | Open |
...
3. The SU(3) structure of the 8-mode multiplet. D₄ orbit.
4. α_s(M_Z) = 0.117900. Honest calculation gives ~0.028.
5. Entropy uniquely determines N/4. All monotone functions work.
6. The 1/180 correction. Replaced by SM top-Yukawa RGE.
7. The Yukawa matrix is the pairing matrix. Mass eigenvalues wrong.
8. The dark photon "falsification." The formulas work with g_dark = g_EM.
9. The "four spin structures give 14/7/7/3" label. It's three properties of one multiplet.
...
python 08_Core_Framework_Utilities/win_8x8_derivation.py
...
│   ├── win_shell_derivation.py
│   └── win_8x8_derivation.py (NEW)
...
win_mass_formula.py	Verifies mass formula
win_shell_derivation.py	Verifies the derived shell rule
win_8x8_derivation.py	NEW: Verifies 2(L−1) = 14 → L = 8
Related Documents
...
WIN kL FULL DERIVATION
Warp Factor kL and the Structure of the WIN Vacuum
WIN to MD Holographic Derivation of the Weinberg Angle
Geometric Higgs VEV & Mass Proof
The Machine Outline — What is WIN
WIN Validation Status — Updated Canonical Validation Record
Interactive widgets
...
The kinetic mixing is derived: ε = 2025/1697500. The 18% tax is identified: (d−1)²/(N−14). A mesh-native mass formula reproduces the SM masses at 10% accuracy with no free parameters.
...
What is open: The mass ordering within classes (11/14), the dark photon mass (0.358 vs 0.291), kL from the mesh, the interaction (SYK couplings), flavor physics (CKM, PMNS), the strong coupling (falsified), and the RF prediction.
